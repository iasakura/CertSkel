...
Lemma gen_seq st len : ret (seq st len) = gen (fun x => ret (st + x)) (Z.of_nat len).
Proof.
Admitted.  

Lemma zip_seq (a1 a2 : list Z) :
  gen (fun i =>
         do! t1 := nth_error a1 i in
         do! t2 := nth_error a2 i in
         ret (t1, t2)) (Z.min (Z.of_nat (length a1)) (Z.of_nat (length a2))) = 
  ret (zip a1 a2).
Admitted. 
