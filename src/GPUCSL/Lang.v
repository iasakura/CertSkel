...
Inductive binop_arith :=
| OP_plus | OP_min | OP_mult | OP_sub | OP_div | OP_mod.
...
Inductive binop :=
| OP_arith : binop_arith -> binop
| OP_comp : binop_comp -> binop
| OP_bool : binop_bool -> binop.
...
| Cread (typ : option CTyp) (x: var) (e: exp)
...
Definition binop_arith_denote op x y :=
  match op with
...
Fixpoint edenote e (s : stack) : option val :=
  match e with
    ...
    | Enum n => Some (VZ n)
    | Eoff loc off =>
      match edenote loc s, edenote off s with
      | Some (VPtr (Loc pl l)), Some (VZ o) => Some (VPtr (Loc pl (l + o)))
      | _, _ => None
      end
    | Eunop op e =>
...
| red_Read: forall x e l ss ss' (cty : option CTyp) s h v
                   (EQ1: ss = (s, h))
                   (Eval: edenote e s = Some (VPtr l))
                   (RD: h l = Some v)
                   (EQ2: ss' = (var_upd s x v, h)),
              (x ::T cty ::= [e]) / ss ==>s Cskip / ss'
| red_Write: forall e1 e2 l v ss ss' s h
                    (EQ1: ss = (s, h))
                    (Eval1: edenote e1 s = Some (VPtr l))
...
| aborts_Read_err: forall x e ty ss
                       (EVAL: edenote e (fst ss) = None),
    aborts (Cread ty x e) ss
| aborts_Read_inv: forall x e n ty ss
                       (EVAL: edenote e (fst ss) = Some (VZ n)),
    aborts (Cread ty x e) ss
| aborts_Read_acc: forall x e l ty ss
                       (EVAL: edenote e (fst ss) = Some (VPtr l))
                       (NIN: snd ss l = None),
    aborts (Cread x ty e) ss
| aborts_Write_err1: forall e1 e2 ss
                       (EVAL: edenote e1 (fst ss) = None),
    aborts (Cwrite e1 e2) ss
| aborts_Write_err2: forall e1 e2 ss
                       (EVAL: edenote e2 (fst ss) = None),
    aborts (Cwrite e1 e2) ss
| aborts_Write_inv: forall e1 e2 n ss
                       (EVAL: edenote e1 (fst ss) = Some (VZ n)),
    aborts (Cwrite e1 e2) ss
| aborts_Write_acc: forall e1 e2 l ss
                        (EVAL: edenote e1 (fst ss) = Some (VPtr l))
                        (NIN: snd ss l = None),
    aborts (Cwrite e1 e2) ss
...
Module PLang.
  ...
  Definition access_ok (c : cmd) (s : stack) (h : pheap) :=
    ...
    | Some (VPtr v) => exists x, this h v = Some x
    | _ => True
    end.

  Definition write_ok (c : cmd) (s : stack) (h : pheap) :=
    match writes c s with
    | Some (VPtr v) => exists x, this h v = Some (full_p, x)
    | _ => True
    end.
...
Module BigStep.
  ...
  | eval_Read : forall (x : var) (e : exp) (l : loc) (cty : option CTyp) (v : val) (st st' : pstate) (s : stack) (h : pheap) (q : Qc),
                  (st = (s, h)) -> 
                  (edenote e s = Some (VPtr l)) ->
                  (this h l = Some (q, v)) ->
                  (st' = (var_upd s x v, h)) ->
                  (x ::T cty ::= [e]) / st || None / st'
  | eval_Write : forall (e1 : exp) (e2 : exp) (ss ss' : pstate) (s : stack) (h : pheap) (l : loc) (v v' : val),
                   (ss = (s, h)) ->
                   (edenote e1 s = Some (VPtr l)) ->
                   (edenote e2 s = Some v) ->
                   this h l = Some (1, v') ->
                   (ss' = (s, ph_upd2 h l v)) ->
...
Section NonInter.
  ...
  | ty_read : forall (v : var) (e : exp) (pc ty : type) (cty : option CTyp),
                typing_exp e ty -> le_type (join ty pc) (g v) = true ->
                typing_cmd (v ::T cty ::= [e]) pc
  | ty_write : forall (e1 : exp) (e2 : exp) (pc : type),
                 typing_cmd ([e1] ::= e2) pc
  ...
  Proof.
    ...
    - inversion ev2; subst; simpl in *; repeat split; [ | intuition eauto].
      ...
      + intros y hlo; pose proof ((proj1 hcomp) y hlo); unfold var_upd; destruct (var_eq_dec y x); eauto; subst.
        ...
        assert (edenote e s = edenote e s0) by (apply low_eq_eq_exp; intuition eauto).
        rewrite H2 in *.
